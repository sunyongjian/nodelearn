...
function cookieParser(){
    return function(req,res,next){
        var cookies = req.headers.cookie;// name=sun; age=6
...
app.get('/write',function(req,res){
    ...
    // 2.设置域名
   // res.cookie('name','sun',{domain:'a.sun.cn'});

    // 3. path 指定此COOKIE所属的路径
    //res.cookie('name','sun',{path:'/visit'});

    // 4.制定过期时间
    //res.cookie('name','sun',{maxAge:20*1000});
   // res.cookie('name','sun',{expires:new Date(Date.now()+20*1000)});
